...
COUNTRY_MAP = {
    ...
    "zimbabwe": "ZW", "belanda": "NL", "jerman": "DE", "perancis": "FR",
    "prancis": "FR", "inggris": "GB", "jepang": "JP", "tiongkok": "CN",
    "korea selatan": "KR", "arab saudi": "SA", "uni emirat arab": "AE",
}

def cari_kode_negara(nama):
    """Convert country name to World Bank 2-letter code."""
...
def buat_grafik_comparison(negara_data):
    ...
    for ax, title in zip(axes, ["GDP (Miliar USD)", "Education Spending (% GDP)"]):
        ax.set_title(title)
        ax.set_xlabel("Tahun")
...
def generate_pdf_single(nama_negara, gdp_terbaru, gdp_growth, gdp_tahun_awal, edu_val, analisis, chart_buf):
    ...
    story.append(Paragraph(f"Generated by ADB Country Intelligence Tool | {datetime.date.today().strftime('%d %B %Y')}", sub_style))
    story.append(Paragraph("Built by Nadhif Fadhlan — MSc Technopreneurship & Innovation, NTU Singapore", sub_style))
...
with st.sidebar:
    st.markdown("### 🌏 ADB Country Intelligence Tool")
    st.markdown("Ketik nama negara dalam **bahasa Indonesia atau Inggris** — tool akan otomatis mendeteksi kodenya.")
    st.divider()
...
if mode == "🔍 Single Country":
    ...
    nama_negara = st.text_input(
        "Nama Negara",
        placeholder="contoh: Indonesia, Jepang, United States..."
    )
    analyze_btn = st.button("🔍 Analisis Sekarang", type="primary", use_container_width=True)

    if analyze_btn:
        if not nama_negara:
            st.error("Masukkan nama negara dulu ya!")
        else:
            kode = cari_kode_negara(nama_negara)
            if not kode:
                st.error(f"Negara '{nama_negara}' tidak dikenali. Coba nama lain, misalnya 'Indonesia', 'Japan', 'Germany'.")
            else:
                st.caption(f"🔎 Kode negara terdeteksi: **{kode}**")
                ...
                if gdp.empty:
                    st.error(f"Data GDP untuk '{nama_negara}' tidak ditemukan.")
                else:
                    gdp_terbaru    = gdp.iloc[-1]["Nilai"] / 1e9
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.caption("Masukkan 2–5 negara. Nama bisa dalam bahasa Indonesia atau Inggris.")
    ...
    if compare_btn:
        ...
        if len(valid_inputs) < 2:
            ...
            for nama in valid_inputs:
                ...
                if not kode:
                    st.warning(f"⚠️ '{nama}' tidak dikenali, dilewati.")
                    continue
                ...
                if not gdp.empty:
                    ...
                    st.warning(f"⚠️ Data {nama} tidak ditemukan, dilewati.")
