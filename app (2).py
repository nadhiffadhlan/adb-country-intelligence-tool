...
def buat_grafik_single(gdp, pendidikan, nama_negara):
    """Single country chart — GDP + Education side by side."""
...
def buat_grafik_comparison(negara_data):
    ...
    negara_data = list of {"nama": str, "gdp": df, "pendidikan": df}
    """
    ...
    for i, nd in enumerate(negara_data):
        ...
        if not gdp.empty:
            axes[0].plot(gdp["Tahun"], gdp["Nilai"] / 1e9,
                         ...
                         linewidth=2, label=nama)

        if not edu.empty:
            axes[1].plot(edu["Tahun"], edu["Nilai"],
                         color=color, marker="o", markersize=3,
                         linewidth=2, label=nama)
...
def generate_pdf_single(nama_negara, gdp_terbaru, gdp_growth, gdp_tahun_awal, edu_val, analisis, chart_buf):
    ...
    story.append(Paragraph(
        ...
        sub_style))
    story.append(Paragraph(
        "Built by Nadhif Fadhlan — MSc Technopreneurship & Innovation, NTU Singapore", sub_style))
    ...
    tbl = Table(table_data, colWidths=[8*rcm, 8*rcm])
    ...
    story.append(Paragraph(
        ...
        "github.com/nadhiffadhlan/adb-country-intelligence-tool", sub_style))
...
def generate_pdf_comparison(negara_data, analisis_regional, chart_buf):
    """PDF for multi-country comparison report."""
    buf = io.BytesIO()
    doc = SimpleDocTemplate(
        buf, pagesize=A4,
        rightMargin=2*rcm, leftMargin=2*rcm,
    ...
    header = ["Country", "GDP (Latest)", "GDP Growth", "Education Spending"]
    rows = [header]
    for nd in negara_data:
        ...
        if not gdp.empty:
            ...
            gdp_str     = f"${gdp_terbaru:.1f}B"
            growth_str  = f"{gdp_growth:.1f}%"
        else:
            gdp_str = growth_str = "N/A"
        edu_str = f"{edu.iloc[-1]['Nilai']:.1f}% GDP" if not edu.empty else "N/A"
        rows.append([nd["nama"], gdp_str, growth_str, edu_str])
...
if mode == "🔍 Single Country":
    ...
    if analyze_btn:
        if not nama_negara or not kode_negara:
            ...
            with st.spinner(f"⏳ Mengambil data {nama_negara}..."):
                gdp        = ambil_data("NY.GDP.MKTP.CD", kode_negara)
                pendidikan = ambil_data("SE.XPD.TOTL.GD.ZS", kode_negara)

            if gdp.empty:
                st.error(f"Data GDP untuk '{kode_negara}' tidak ditemukan.")
            else:
                gdp_terbaru    = gdp.iloc[-1]["Nilai"] / 1e9
                ...
                with st.spinner("Menyiapkan PDF..."):
                    chart_pdf = buat_grafik_single(gdp, pendidikan, nama_negara)
                    pdf_buf = generate_pdf_single(
                        nama_negara, gdp_terbaru, gdp_growth,
                        gdp_tahun_awal, edu_val, analisis, chart_pdf
                    )
                st.download_button(
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.caption("Masukkan 2–5 negara untuk dibandingkan dalam satu grafik")

    jumlah = st.slider("Jumlah negara", min_value=2, max_value=5, value=3)

    negara_inputs = []
    cols = st.columns(jumlah)
    for i, col in enumerate(cols):
        with col:
            nama = st.text_input(f"Negara {i+1}", placeholder="Indonesia", key=f"nama_{i}")
            kode = st.text_input(f"Kode {i+1}", placeholder="ID", key=f"kode_{i}").upper()
            negara_inputs.append((nama, kode))

    compare_btn = st.button("🌐 Bandingkan Sekarang", type="primary", use_container_width=True)

    if compare_btn:
        valid = [(n, k) for n, k in negara_inputs if n and k]
        if len(valid) < 2:
            st.error("Isi minimal 2 negara ya!")
        else:
            negara_data = []
            for nama, kode in valid:
                with st.spinner(f"⏳ Mengambil data {nama}..."):
                ...
                if not gdp.empty:
                    ...
                    st.warning(f"⚠️ Data {nama} ({kode}) tidak ditemukan, dilewati.")

            if len(negara_data) < 2:
                st.error("Minimal 2 negara harus punya data valid.")
            else:
                # Comparison table
                st.subheader("📊 Perbandingan Indikator Ekonomi")
                rows = []
                for nd in negara_data:
                    gdp = nd["gdp"]
                    edu = nd["pendidikan"]
                    ...
                    rows.append({
                        "Negara": nd["nama"],
                        "GDP Terbaru (B USD)": f"${gdp_terbaru:.1f}",
                        "GDP Growth": f"{gdp_growth:.1f}%",
                        "Education Spending": edu_str
                    })
                ...
                with st.spinner("AI lagi nulis analisis regional..."):
                    prompt = f"""
                    Kamu adalah senior regional economist di ADB.
                    Bandingkan ekonomi negara-negara berikut dalam 2 paragraf:
                    {negara_summary}
                    
                    Fokus: siapa yang paling kuat, siapa yang paling cepat tumbuh, 
                    apa implikasi untuk strategi investasi ADB di kawasan ini.
                    Bahasa Indonesia, formal tapi accessible.
                    """
                    analisis_regional = tanya_ai(prompt, api_key)
                st.info(analisis_regional)

                # PDF Download
                st.divider()
                with st.spinner("Menyiapkan PDF..."):
                    chart_pdf = buat_grafik_comparison(negara_data)
                    pdf_buf = generate_pdf_comparison(negara_data, analisis_regional, chart_pdf)

                negara_str = "_vs_".join([nd["nama"].replace(" ", "") for nd in negara_data])
                st.download_button(
                    label="⬇️ Download PDF Regional Report",
                    data=pdf_buf,
                    file_name=f"ADB_Regional_{negara_str}_{datetime.date.today()}.pdf",
                    mime="application/pdf",
                    use_container_width=True,
                    type="primary"
                )
